...
int numberOfSides = int(showInputDialog("entrer le nombre de faces"));//<<< change this to change the size of the cube
...
float rotationSpeed = int(showInputDialog("vitesse de rotaion du rubiks cube"));
...
void draw() {
  if (!actualPause) {
    ...
    if (cube.algos.stageNo == 7) {
       frameToStop++;
       if(frameToStop == 7) {
         showMessageDialog(null, "Resolu " + solveCounterThing + " rubik's cube(s) en " +(cube.algos.globalTimer/1000.0)+ " seconde(s)", "Info" , PLAIN_MESSAGE);
         exit();
       }
    }
